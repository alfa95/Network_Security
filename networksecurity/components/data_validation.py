...
class DataValidation:
    ...
    def validate_number_of_columns(self, datafram: pd.DataFrame) -> bool:
        try:
            ...
            logging.info(f"Number of columns in the dataset: {len(datafram.columns)}")
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            status = self.validate_number_of_columns(datafram=train_dataframe)
            if not status:
                error_message = "Number of columns in the schema and Train dataset are not matching"
            status = self.validate_number_of_columns(datafram=test_dataframe)
            if not status:
                error_message = "Number of columns in the schema and Test dataset are not matching"
